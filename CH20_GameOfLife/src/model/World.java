...
public class World {
	...
	public World(int rows, int columns) { // 월드 생성자(가로,세로)
		...
		grid = new boolean[rows][columns]; // 이중배열 객체만들기(크기설정)
	...
	public void randomize() {
		...
		for (int i = 0; i < (rows * columns) / 10; i++) { // 횟수는 줄과 열이 많으면 많아진다.
			int row = random.nextInt(rows); // 랜덤으로 0부터 rows(전체줄수)-1까지 랜덤생성
	...
	public void clear() {
		...
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < columns; col++) {
				setCell(row, col, false); // 모든 셀을 false로
			}
		}
	}

	public void next() {
		// 스페이스바를 누르면 모든 셀의 좌표를 출력
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < columns; col++) {
				int neighbours = countNeighbours(row, col);
				// System.out.printf("( %d, %d ) %d \n", row, col, neighbours);
				/*
				 * 알고리즘 1. 주변 녹색개수 < 2 or > 3 은 검은색 2. 녹색개수 == 3 일때 녹색 3. 녹색개수 == 2 일때 그대로
				 */
				...
				buffer[row][col] = status; // 임시 배열 buffer에 입력
			}
		}
		...
		for (int rowOffset = -1; rowOffset <= 1; rowOffset++) {
			for (int colOffset = -1; colOffset <= 1; colOffset++) {
				...
				if (rowOffset == 0 && colOffset == 0) {
					continue; // 본인셀의 갯수는 세지않고 다시 반복한다.
				}
